...
func (h *GatewayHandler) handleInitialize(req *JSONRPCRequest) *JSONRPCResponse {
	return &JSONRPCResponse{
		...
		Result: map[string]interface{}{
			...
			"instructions": "To analyze a LOCAL image: if it is small (roughly <= 10KB), inline it as base64 directly to vision.analyze_image; otherwise call vision.upload_image with local_path to get OS-matched upload_command + file_url, run the command for YOUR system via your shell (on Windows PowerShell use the curl.exe variant — bare `curl` is an alias for Invoke-WebRequest there and will fail; no API key needed), then call vision.analyze_image with image_url=file_url. Never paste large image base64 into tool arguments.",
		},
	}
}
