...
func GetCallLogStats(filter *LogFilter) (*LogStatsResult, error) {
	...
	db.Select("COALESCE(AVG(duration_ms), 0)").Scan(&avgDuration)
...
func GetCallLogStatsByUser(userID int64, filter *LogFilter) (*LogStatsResult, error) {
	...
	baseQuery := DB.Model(&McpCallLog{}).Where("user_id = ?", userID)
	...
	var successCount int64
	DB.Model(&McpCallLog{}).Where("user_id = ?", userID).Scopes(func(db *gorm.DB) *gorm.DB {
		return applyLogFilter(db, userFilter)
	}).Where("response_status = ?", "success").Count(&successCount)

	var avgDuration float64
	db := DB.Model(&McpCallLog{}).Where("user_id = ?", userID)
	db = applyLogFilter(db, userFilter)
	db.Select("COALESCE(AVG(duration_ms), 0)").Scan(&avgDuration)

	today := time.Now().Truncate(24 * time.Hour)
	var todayCount int64
	DB.Model(&McpCallLog{}).Where("user_id = ? AND created_at >= ?", userID, today).Count(&todayCount)
