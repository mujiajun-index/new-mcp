...
### 4.12 KeySelector — 多秘钥选择器(仅 HTTP 类传输)
...
> 层面发生——`SDKAdapter.CallWithMeta` 每次逻辑调用 Pick 一次经 ctx 传给 RoundTripper
