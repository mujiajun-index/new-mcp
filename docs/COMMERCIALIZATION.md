...
> **虚拟工具**(vision/camera)属用户自有(`source=user` 性质),保持免费;若未来上架为市场服务则另议。
...
| 既有能力 | 本期变化 |
|----------|----------|
| 管理员上架市场项([service/marketplace.go](../service/marketplace.go)) | **手动添加** 或 **从自有服务克隆**(深拷贝,无关联,凭证保留并提示替换);配置上游 transport(**平台托管**)+ **服务级定价**;**非自用模式必须显式定价**(§5.6) |
