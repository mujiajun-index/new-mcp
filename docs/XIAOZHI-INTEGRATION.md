...
1. **向小智云提供工具（核心模式）**: NewMCP 主动连接小智云 WSS，把平台上注册的所有 MCP 工具暴露给小智设备使用
2. **消费小智云的工具**: NewMCP 注册小智 WSS 端点作为上游 MCP 服务，让 LLM 客户端 (Claude Code 等) 也能调用小智自身的工具
...
**步骤 3: 在 NewMCP 平台配置小智 WSS 链接**

用户从小智云平台复制 WSS 注册链接，粘贴到 NewMCP 平台：

```json
POST /api/v1/xiaozhi/endpoints
{
    "name": "我的小智 Agent",
...
新增 `xiaozhi_endpoints` 表（替代原来 `devices` 表的小智部分）：
...
CREATE TABLE `xiaozhi_endpoints` (
    ...
    `agent_id_ext`     VARCHAR(128)    DEFAULT '' COMMENT '小智 Agent ID (从 JWT 解析)',
    ...
    -- 绑定分组
    `group_id`         BIGINT UNSIGNED DEFAULT NULL COMMENT '要暴露给小智的 MCP 分组',

    -- 连接状态
    `auto_connect`     TINYINT         DEFAULT 1 COMMENT '是否自动连接',
    `connection_status` VARCHAR(16)    DEFAULT 'disconnected' COMMENT 'connected, disconnected, error',
    `last_connected_at` DATETIME       DEFAULT NULL,
    `last_error`       TEXT            DEFAULT '' COMMENT '最近一次连接错误',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='小智 WSS 端点配置';
```
